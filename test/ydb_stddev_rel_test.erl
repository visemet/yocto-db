...
%% @doc This module tests STDDEV aggregate node for relations.
-module(ydb_stddev_rel_test).
...
test_setup_helper(Answer, N) ->
  ...
  , {ok, DummyPid} = ydb_file_input:start_link([
    ...
    % The aggregate node
  , {ok, AggrPid} = ydb_stddev_rel:start_link([
        {column, num}
    ], [{listen, [DummyPid]}])
  , Listener = spawn(
        ?MODULE
...
get_diff_tuples() ->
    ...
    Table1 = {[{ydb_tuple,2,{3}}],[{ydb_tuple,0,{0}}]}
  , Table2 = {[{ydb_tuple,3,{4}}],[{ydb_tuple,2,{3}}]}
  , Table3 = {[{ydb_tuple,4,{2}}],[{ydb_tuple,3,{4}}]}
...
get_diff_tables() ->
  ...
  , ets:insert(Diff3, {'+', {row, 4}, {ydb_tuple, 4, {16}}})
