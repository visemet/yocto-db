...
%% @doc This module tests VAR aggregate node for relations.
-module(ydb_var_rel_test).
...
test_setup_helper(Answer, N) ->
  ...
  , {ok, DummyPid} = ydb_file_input:start_link([
    ...
    % The aggregate node
  , {ok, AggrPid} = ydb_var_rel:start_link([
        {column, num}
    ], [{listen, [DummyPid]}])
  , Listener = spawn(
        ?MODULE
...
get_diff_tuples() ->
    ...
    Table1 = {[{ydb_tuple,2,{9}}],[{ydb_tuple,0,{0}}]}
  , Table2 = {[{ydb_tuple,3,{14}}],[{ydb_tuple,2,{9}}]}
  , Table3 = {[{ydb_tuple,4,{4}}],[{ydb_tuple,3,{14}}]}
