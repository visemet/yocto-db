...
%% @doc This module tests AVG aggregate node for relations.
...
test_setup_helper(Answer, N) ->
  ...
  , {ok, DummyPid} = ydb_file_input:start_link([
    ...
    % The aggregate node
  , {ok, AggrPid} = ydb_avg_rel:start_link([
        {column, num}
    ], [{listen, [DummyPid]}])
  , Listener = spawn(
        ?MODULE
...
get_diff_tuples() ->
    ...
    Table1 = {[{ydb_tuple,2,{6}}],[{ydb_tuple,0,{0}}]}
  , Table2 = {[{ydb_tuple,3,{8}}],[{ydb_tuple,2,{6}}]}
  , Table3 = {[{ydb_tuple,4,{7}}],[{ydb_tuple,3,{8}}]}
