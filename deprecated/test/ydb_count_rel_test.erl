...
%% @doc This module tests COUNT aggregate node for relations.
...
test_setup_helper(Answer, N) ->
  ...
  , {ok, DummyPid} = ydb_file_input:start_link([
    ...
    % The aggregate node
  , {ok, AggrPid} = ydb_count_rel:start_link([
        {column, num}
    ], [{listen, [DummyPid]}])
  , Listener = spawn(
        ?MODULE
...
get_diff_tuples() ->
    ...
    Table1 = {[{ydb_tuple,4,{3}}],[{ydb_tuple,0,{0}}]}
  , Table2 = {[{ydb_tuple,2,{-1}}],[{ydb_tuple,0,{0}}]}
  , Table12 = {[{ydb_tuple,2,{2}}],[{ydb_tuple,4,{3}}]} % apply 1, then 2
  , Table1and2 = {[{ydb_tuple,4,{2}}],[{ydb_tuple,0,{0}}]} % apply together
